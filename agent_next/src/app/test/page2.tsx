...
const Independent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(progressInterval); clearTimeout(loadTimer); };
  }, [isClient]);
    ...
    <div className={`${styles.loadingContainer}${!isLoading ? ` ${styles.loadingFadeOut}` : ''}`}>
      <div className={styles.loadingContent}>
        <div className={styles.loadingLogo}>
          <img src="/assets/logo.jpg" draggable={false} alt="logo" />
          <span>AI扫地机器人助手</span>
        </div>
        <div className={styles.progressBarContainer}>
          <div className={styles.progressBarTrack}>
            <div className={styles.progressBarFill} style={{ width: `${loadingProgress}%` }} />
          </div>
          <div className={styles.progressText}>{Math.round(loadingProgress)}%</div>
        </div>
        <div className={styles.loadingTips}>{loadingText}</div>
      </div>
    </div>
  );
